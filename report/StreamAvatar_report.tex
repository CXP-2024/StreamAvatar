...
The optimized teacher pipeline is close to the real-time boundary on a B200 GPU, especially after warm-up and with batched rendering, but it should not be interpreted as an order-of-magnitude faster-than-real-time system. The larger acceleration comes from the student, which replaces the expensive teacher motion rollout with direct blockwise motion prediction. A real streaming demo also needs warm-up, rolling audio context, rolling motion history, frame buffering, and stable playback. Our current web prototype validates the basic form: models are loaded before inference, audio chunks are processed incrementally, rendered frames are queued, and the frontend plays frames continuously.
